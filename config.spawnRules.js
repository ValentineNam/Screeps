module.exports = function(deps) {
    ...
    const rules = [
        ...
        {
    role: 'distributor',
        bodyRole: 'logist', // убедитесь, что у вас есть body-профиль 'distributor'
        priority: 80, // чуть ниже, чем у логиста (85)
        condition: (ctx) => {
        ...
        //             return mem.tasks.some(t => t.type === 'transfer' && t.status === 'pending');
        ...
        //             const mem = (Memory.chemistry && Memory.chemistry.rooms && Memory.chemistry.rooms[ctx.roomName]) ? Memory.chemistry.rooms[ctx.roomName] : null;
        //             if (!mem || !mem.tasks) return false;
        //             return mem.tasks.some(t => t.type === 'produce' && ['staged','ready_for_lab','in_progress'].includes(t.status));
        //         } catch (e) { return false; }
        //     },
